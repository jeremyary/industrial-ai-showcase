...
### 7. Service Mesh — **code landed in Part E; injection disabled pending hub CA reconciliation**
PeerAuthentication manifest + namespace comments are in place. The namespace is **not currently labeled** `istio.io/rev: default` because the hub's `default` revision has a CA drift: `istio-ca-secret` (signing cert) rotated on 2026-04-17 but `istio-ca-root-cert` ConfigMaps across the cluster still hold the 2026-03-23 cert. Sidecar proxies can't verify istiod's serving cert → `x509: certificate signed by unknown authority`. Restarting istiod and deleting the ConfigMap both fail to re-propagate — istiod isn't reconciling after rotation. This is a pre-existing hub issue affecting every mesh-enrolled namespace.

Re-enable after the hub mesh CA is reconciled — set `istio.io/rev: default` on the namespace, bounce Nucleus pods, and they'll get sidecars + mTLS. PeerAuthentication PERMISSIVE is already applied, ready for that day. `ngc-pullsecret-render` Job already opts out of injection.
